...
struct SearxResponse { results: Vec<SearxItem> }
...
pub fn search(query: &str) -> Result<Vec<SearchResult>, String> {
    let client = reqwest::blocking::Client::builder()
        ...
        .build().map_err(|error| format!("Could not create SearXNG client: {error}"))?;
    let configured = std::env::var("AI_HARNESS_SEARXNG_URL").ok();
    let instances = configured.iter().map(String::as_str).chain(DEFAULT_INSTANCES.iter().copied());
    let mut last_error = "No SearXNG instance responded.".to_string();

    for instance in instances {
        let endpoint = format!("{}/search", instance.trim_end_matches('/'));
        match client.get(&endpoint).query(&[("q", query), ("format", "json"), ("language", "en-US")]).send() {
            Ok(response) if response.status().is_success() => match response.json::<SearxResponse>() {
                Ok(payload) => {
                    let results = payload.results.into_iter().filter_map(|item| {
                        let url = item.url?;
                        if super::scraper::is_safe_external_url(&url) {
                            Some(SearchResult { title: item.title.unwrap_or_default(), url, snippet: item.content.unwrap_or_default(), content: String::new() })
                        } else { None }
                    }).take(8).collect::<Vec<_>>();
                    if !results.is_empty() { return Ok(results); }
                    last_error = format!("{instance} returned no usable results.");
                }
                Err(error) => last_error = format!("Could not parse SearXNG response from {instance}: {error}"),
            },
