...
const SAFETY_MARGIN_PERCENT: u32 = 5;
...
impl ConversationMemory {
    pub fn from_summary(summary: String) -> Self {
        Self { summary, summarized_messages: HashSet::new() }
    }
    ...
    F: FnMut(super::runtime::GenerationEvent) -> Result<(), String>,
{
    ...
    for continuation in 0..=MAX_AUTO_CONTINUATIONS {
        if continuation == 0 {
            emit(super::runtime::GenerationEvent::Status("Checking conversation context".to_string()))?;
        } else {
            emit(super::runtime::GenerationEvent::Status("Continuing response".to_string()))?;
        }
        let prepared = prepare_request(engine, &history, requested_tokens, temperature, memory, time_context)?;
        if !prepared.dropped.is_empty() {
            emit(super::runtime::GenerationEvent::Status("Compacting earlier conversation".to_string()))?;
            compact_dropped_messages(engine, memory, &prepared.dropped, time_context, &should_cancel)?;
        }
        ...
        let prepared = prepare_request(engine, &history, requested_tokens, temperature, memory, time_context)?;
        if !prepared.dropped.is_empty() {
            emit(super::runtime::GenerationEvent::Status("Updating conversation memory".to_string()))?;
            compact_dropped_messages(engine, memory, &prepared.dropped, time_context, &should_cancel)?;
        }
        let prepared = prepare_request(engine, &history, requested_tokens, temperature, memory, time_context)?;

        emit(super::runtime::GenerationEvent::Status("Writing response".to_string()))?;
        ...
        match result.finish_reason {
            FinishReason::Stop | FinishReason::Cancelled | FinishReason::RepetitionDetected => {
                return Ok(GenerationResult { content: full_output, finish_reason: result.finish_reason });
            }
            FinishReason::Length if continuation == MAX_AUTO_CONTINUATIONS => {
                return Ok(GenerationResult { content: full_output, finish_reason: FinishReason::Length });
            }
            FinishReason::Length => {
                ...
                history.push(ChatMessage { role: "assistant".to_string(), content: result.content, created_at: None });
...
fn prepare_request(
    ...
    let safety_margin = context * SAFETY_MARGIN_PERCENT / 100;
    let target_response = requested_tokens.clamp(MIN_RESPONSE_TOKENS, MAX_RESPONSE_TOKENS).max(DEFAULT_RESPONSE_TOKENS);
    let prompt_budget = context.saturating_sub(safety_margin + target_response);

    let mut kept = vec![time_manager::system_message(time_context)];
    kept.extend(history.iter().filter(|message| message.role == "system").cloned());
    if !memory.summary.is_empty() {
        kept.push(ChatMessage {
            role: "system".to_string(),
            content: format!("Conversation memory (private, compact):\n{}", memory.summary),
            created_at: None,
        });
    }
    let mut used = engine.count_messages_tokens(&kept);
    let non_system = history.iter().filter(|message| message.role != "system").cloned().collect::<Vec<_>>();
    let mut newest_first = Vec::new();
    let mut dropped = Vec::new();

    for (index, message) in non_system.iter().enumerate().rev() {
        let cost = engine.count_message_tokens(message);
        if !newest_first.is_empty() && used.saturating_add(cost) > prompt_budget {
    ...
    let hard_prompt_limit = context.saturating_sub(safety_margin + MIN_RESPONSE_TOKENS);
    while engine.count_messages_tokens(&kept) > hard_prompt_limit {
        let Some(index) = kept.iter().position(|message| message.role != "system") else {
            // A stale memory block must never crowd out the user's newest turn.
            if let Some(memory_index) = kept.iter().position(|message| message.content.starts_with("Conversation memory (private, compact):")) {
                kept.remove(memory_index);
                continue;
            }
            break;
        };
        if kept.iter().filter(|message| message.role != "system").count() > 1 {
            dropped.push(kept.remove(index));
            continue;
        }
        let message = &mut kept[index];
        let shortened = truncate_tail(&message.content, message.content.len().saturating_mul(3) / 4);
    ...
    let remaining = context.saturating_sub(safety_margin + actual_prompt_tokens);
    let max_tokens = remaining.clamp(MIN_RESPONSE_TOKENS, MAX_RESPONSE_TOKENS).min(requested_tokens.max(MIN_RESPONSE_TOKENS));
    Ok(PreparedRequest {
        request: ChatRequest { messages: kept, max_tokens: Some(max_tokens), temperature, session_id: None },
        dropped,
    })
}
...
fn compact_dropped_messages(
    ...
    let new_messages = dropped.iter().filter(|message| !memory.summarized_messages.contains(&message_hash(message))).cloned().collect::<Vec<_>>();
    ...
    let source = new_messages.iter().map(|message| {
        let timestamp = message.created_at.as_deref().unwrap_or("timestamp unavailable");
    ...
    if result.finish_reason == FinishReason::Stop && !result.content.trim().is_empty() {
        ...
        memory.summarized_messages.extend(new_messages.iter().map(message_hash));
    }
    Ok(())
}
